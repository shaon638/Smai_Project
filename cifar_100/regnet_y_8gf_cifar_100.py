...
def train(model,num_epochs,train_loader,val_loader,optimizer,criterion, model_name = "RegNet_y_8gf_cifar100", model_path = "/ssd_scratch/cvit/shaon/teacher_model_regnetv2",loss_path= "loss_2", device = "cuda"):
	train_loss = {}
	val_loss =  {}
	dur = []
	...
	for epoch in range(num_epochs):
		...
		with open(loss_path + "/val_loss"+"_"+model_name+"_"+str(num_epochs)+".pkl","wb")as file:
			dump(val_loss, file)
		# with open(loss_path+ "/info_about_model"+"_"+model_name+"_"+str(epoch +1)+".txt","wb")as file:
		# 	file.write(lines)


device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#initialize the pretrained model 
teacher_model = models.regnet_y_8gf(pretrained = False)
...
num_epochs = 1000
train(teacher_model, num_epochs, train_loader, val_loader, optimizer, criterion)
